...
class ueEnv(gymnasium.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-1000,high=1000,shape=(1,135))
        ...
        self.start_pos = [(9.2,-33.6,-25), (-2,42.4,-25), (-44.1,38,-25), 
                 ...
                 (-51,1,-25),(-38,-1,-25),(-20,45,-25),(-38,-22,-25)]
    ...
    def step(self, action):
        ...
        nx_latent = self.cmvae(nx_obs)
        nx_latent = nx_latent.detach().cpu().numpy()
        nx_state = np.concatenate((nx_latent,nx_info), axis= 1)
        ...
        return nx_state, reward, done, truncated ,terminated_info
    ...
    def reset(self, seed=None, options=None):
        ...
        latent = self.cmvae(obs)
        latent = latent.detach().cpu().numpy()
        state = np.concatenate((latent,info), axis= 1)
        dict = {}
        return np.array(state).astype(np.float32), dict
